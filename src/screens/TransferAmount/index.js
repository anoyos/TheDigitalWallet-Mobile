...
export default function TransferAmount({navigation}) {
  ...
  const [currencySymbol, setCurrencySymbol] = useState(
    transfer.fromSymbol || user.currency.symbol,
  );
  const [displayCurrencySymbol, setDisplayCurrencySymbol] = useState(
    currencySymbol,
  );
  const [changeCurrencySymbol, setChangeCurrencySymbol] = useState(
    user.currency.symbol,
  );
  const [availableBalance, setAvailableBalance] = useState(
    user[`${currencySymbol}Balance`],
  );

  useEffect(() => {
    api
      .getExchangeRate({from: currencySymbol, to: user.currency.symbol})
  ...
  const handleNext = async () => {
    ...
    const payload = {
      amount: (
        Math.round(checkedAmount * 10 ** currencies[currencySymbol].decimals) /
        10 ** currencies[currencySymbol].decimals
      ).toString(),
  ...
  const handleChangeDisplayCurrency = () => {
    if (displayCurrencySymbol !== user.currency.symbol) {
      const newAmount = (
        Math.round(
          amount *
            exchangeRate *
            10 ** currencies[user.currency.symbol].decimals,
        ) /
        10 ** currencies[user.currency.symbol].decimals
      ).toString();
      if (
        (!!newAmount.split('.')[1] &&
          newAmount.split('.')[1].length ===
      ...
      setDisplayCurrencySymbol(user.currency.symbol);
    } else {
      const newAmount = (
        Math.round(
          (amount / exchangeRate) * 10 ** currencies[currencySymbol].decimals,
        ) /
        10 ** currencies[currencySymbol].decimals
      ).toString();
      if (
        (!!newAmount.split('.')[1] &&
          newAmount.split('.')[1].length ===
            currencies[currencySymbol].decimals) ||
        newAmount.length >= 7
      )
        setDisabled(true);
      else setDisabled(false);
  ...
  const handleNumberPress = number => {
    ...
    if (
      (!!newNumber.split('.')[1] &&
        newNumber.split('.')[1].length >=
          currencies[displayCurrencySymbol].decimals) ||
      newNumber.length >= 7
    )
      setDisabled(true);
    else setDisabled(false);
  ...
  const disableButton = () => {
    if (
      ...
      amount / exchangeRate > user[`${currencySymbol}Balance`]
    )
      return true;
    if (
      displayCurrencySymbol === currencySymbol &&
      amount > user[`${currencySymbol}Balance`]
    )
      return true;
  ...
  return (
    <SafeAreaView
      ...
      style={{flex: 1}}>
      <SubHeader
        title={`Transfering ${currency.symbol}`}
        ...
        <View
          ...
          <View style={{flexDirection: 'row', alignItems: 'baseline'}}>
            <Text
              style={[
                styles.amount,
                {fontSize: fontSizer(percentWidth(13))},
              ]}>{`${format(
              amount,
              user.locales,
              currencies[displayCurrencySymbol].decimals,
            ...
            <Text
              style={[
                styles.character,
                {fontSize: fontSizer(percentWidth(6.5))},
              ]}>{` ${currencies[displayCurrencySymbol].displaySymbol}`}</Text>
          </View>
            ...
            <View
              ...
              <Button
                ...
                <Text
                  ...
                  {currencies[changeCurrencySymbol].displaySymbol}
                </Text>
              </Button>
            </View>
          )}
          ...
          <View style={{width: '63%', justifyContent: 'center'}}>
            ...
            {currencySymbol !== user.currency.symbol && (
              <Text
                ...
                {`1 ${currencies[currencySymbol].displaySymbol} ≈ ${localize(
                  exchangeRate,
                  user.locales,
                  currencies[user.currency.symbol].decimals,
